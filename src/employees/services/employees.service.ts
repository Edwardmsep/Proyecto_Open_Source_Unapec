...
export class EmployeesService {
    constructor(
      ...
      async update(employeesId: string, employeesDto: EmployeesDto): Promise<employeesDocument> {
        const employeesDocument = new this.employeesModel(employeesDto);
        return await this.employeesModel.findByIdAndUpdate(employeesId, employeesDocument, {
          new: true,
        });
      }
    
      async delete(employeesId: string): Promise<employeesDocument> {
        return await this.employeesModel.findByIdAndDelete(employeesId);
    }
}
